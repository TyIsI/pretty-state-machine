...
function PrettyStateMachine () {
  ...
  if (typeof localStorage !== 'undefined' && localStorage.getItem('pretty-state-machine') !== null) {
    console.log('pretty-state-machine', 'trying to load data from localStorage')

    try {
      const store = JSON.parse(localStorage.getItem('pretty-state-machine'))
      this.store = { ...store, ...{ state: store } }
      console.log('pretty-state-machine', 'loaded data from localStorage')
    } catch (e) {
      console.log('pretty-state-machine', 'failed to load data from localStorage:', e)
    }
  }
}
...
if (typeof localStorage !== 'undefined') {
  console.log('pretty-state-machine', 'saving to localStorage')
  stateMachine.sub((store) => {
    console.log('pretty-state-machine', 'saved to localStorage')
