...
var core_default = stateMachine;

// src/react.ts
init_cjs_shims();
var import_react = __toESM(require_react());
var useStateMachine = (topic, defaultValue) => {
  const [_topicValue, _topicUpdater] = (0, import_react.useState)(defaultValue);
  const updateReceiver = (topicUpdate) => {
    if (topicUpdate[topic] != null) {
      _topicUpdater(topicUpdate[topic]);
    }
  };
  (0, import_react.useEffect)(() => {
    core_default.sub(topic, updateReceiver);
    return () => {
      core_default.unsub(topic, updateReceiver);
    };
  }, []);
  (0, import_react.useEffect)(() => {
    core_default.pub(topic, _topicValue);
  }, [_topicValue]);
